...
class ReturnController(
    ...
    @PostMapping("/returns/{returnOrderId}/items/{returnOrderItemId}/qc/status")
    fun updateQCStatus(@RequestBody updateQCRequest: UpdateQCRequest, @PathVariable returnOrderId: String, @PathVariable returnOrderItemId: String) {
        return returnOrderService.updateItemStatus(returnOrderId, returnOrderItemId, updateQCRequest.status)
    }

    data class UpdateQuantityRequest(val quantity: Int)
    @PostMapping("/returns/{returnOrderId}/items/{returnOrderItemId}/qc/quantity")
    fun updateQuantity(@RequestBody updateQuantityRequest: UpdateQuantityRequest, @PathVariable returnOrderId: String, @PathVariable returnOrderItemId: String) {
        return returnOrderService.updateItemQuantity(returnOrderId, returnOrderItemId, updateQuantityRequest.quantity)
    }
